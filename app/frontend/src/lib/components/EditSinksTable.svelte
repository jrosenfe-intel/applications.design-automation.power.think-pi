<script>
	...
	let currentInput;
	const inputType = ['number', 'text', 'text', 'number', 'number', 'text'];
					...
					<td class="py-1 text-center align-middle">
						...
						{:else}
							...
							>
								...
								{#each ['Equal current', 'Equal voltage', 'Unequal current'] as opt (opt)}
									{#if opt !== compData[header][vidx]}
